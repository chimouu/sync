...
// Get all reviews of a movie by title
...
async function addReview(req, res) {
  try {
    const { _id, title, author, content, rating } = req.body;
    const newReview = new Review({ _id, title, author, content, rating });
    await newReview.save();
    res.status(201).json({ message: 'Review added successfully', id: newReview._id });
  } catch (error) {
    console.error(error);
